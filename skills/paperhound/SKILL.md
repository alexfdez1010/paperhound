---
name: paperhound
description: Search, inspect, download, and convert academic papers (arXiv + Semantic Scholar) to Markdown via the `paperhound` CLI. Use whenever the user asks to find papers, fetch a paper's abstract, download a PDF, or turn a paper into Markdown.
---

# paperhound — paper search and conversion CLI

`paperhound` is a small command-line tool that queries arXiv and Semantic
Scholar in parallel, resolves identifiers (arXiv id, DOI, Semantic Scholar
paper id, or any paper URL), downloads the PDF, and converts it to Markdown
...
```bash
paperhound search "<query>" [--limit N] [--year-min YYYY] [--year-max YYYY] [--source arxiv|semantic_scholar] [--json]
```

- Default `--limit` is 10. Cap it (e.g. `-n 5`) when the user asks for "a few".
- `--source` is repeatable; omit it to query everything.
- JSON output is a list of objects with: `title`, `authors[]`, `abstract`,
  `year`, `venue`, `url`, `pdf_url`, `citation_count`,
  `identifiers.{arxiv_id,doi,semantic_scholar_id}`, `sources[]`.
...
- Don't pass user-controlled strings as `--source` values without restricting
  them to `arxiv` or `semantic_scholar`.
