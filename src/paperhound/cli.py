...
def _run_citation_command(
    ...
    from paperhound.citations import Source, fetch_citations, fetch_references
    ...
    resolved_source: Source | None = None
    if source is not None:
        lower = source.lower().replace("-", "_")
        if lower in ("openalex", "oa"):
            resolved_source = "openalex"
        elif lower in ("semantic_scholar", "s2"):
            resolved_source = "semantic_scholar"
        else:
            raise typer.BadParameter(
                f"Unknown --source {source!r}. Use 'openalex' or 'semantic_scholar'."
            )

    try:
        if mode == "refs":
            papers = fetch_references(identifier, depth=depth, limit=limit, source=resolved_source)
        else:
            papers = fetch_citations(identifier, depth=depth, limit=limit, source=resolved_source)
    except PaperhoundError as exc:
        _exit_on_error(exc)
        return
...
def refs(
    ...
    """
    _run_citation_command(identifier, depth, limit, source, json_output, mode="refs")
